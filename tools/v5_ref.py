"""v5_ref — referentie-implementatie voor de TypeScript-port.

Dit is v5.py, regel voor regel, met precies één normalisatie:

    v5.py laat spelbeslissingen afhangen van CPython's set-iteratievolgorde
    (`for x in self.alive`, `max(useless, ...)`, `for c in self.conn()`).
    Die volgorde is een implementatiedetail van de interpreter, niet van het
    spel: hij verschuift met Python-versie en met de invoeg-/verwijdergeschiedenis
    van de set. Hier wordt elke zulke iteratie vervangen door de bordvolgorde
    (ALL: centrum, ring 1, ring 2, ring 3).

Verder is er niets veranderd. tools/compare_v5.py laat zien dat deze normalisatie
de gemeten uitkomsten niet verschuift; tests/parity.test.ts legt de TS-engine
zet voor zet naast dit bestand.
"""
import random
...
def order(cells):
    """Canonieke bordvolgorde i.p.v. set-volgorde."""
    return sorted(cells, key=lambda c: IDX.get(c, 10**6))
...
class Game:
        ...
        while stk:
            ...
            for x in nb(c):
                if (self.w[x] > 0 or x in self.marks) and x not in seen \
                   and x in self.alive:
                    stk.append(x)
        return order(seen)                                   # NORMALISATIE
    ...
    def laatste_turn(self):
        ...
        for _ in range(self.cfg.get('acts', 2)):
            did = False
            if self.stock <= 1:
                useless = [c for c in order(self.alive) if self.w[c] >= 1   # NORMALISATIE
                           and c != self.seat
            ...
            if did: continue
            if self.stock <= 3:
                half = [x for x in order(self.alive) if self.w[x] == 1        # NORMALISATIE
                        and x != self.seat
    ...
    def nexus_turn(self):
        ...
        if moved == 0:
            cand = [x for x in order(self.alive)                              # NORMALISATIE
    ...
    def consume(self, c):
        ...
        if c in self.marks:
            self.marks.discard(c); self.pile_l -= 1
...
if __name__ == '__main__':
    for lbl, cfg in [('basis 12/28', {}), ('10/26', dict(need_l=10, need_n=26)),
                     ('14/30', dict(need_l=14, need_n=30))]:
        C, T, L, N, F = run(400, **cfg)
        tot = sum(C.values())
        d = {k: round(100*v/tot) for k, v in C.items()}
        print(f"{lbl:20s} mediaan {st.median(T):3.0f}b ({min(T)}-{max(T)}) | {d}")
